            ...
            <div className='px-6'>
                <div className={"flex flex-col items-center justify-center mt-4 font-Philosopher"}>
                    ...
                    <button onClick={() => setVisible(!visible)}
                            className={`rounded bg-[${props.data.colorCode}] text-white opacity-40 hover:opacity-80 w-full mx-4 py-2 border hover:text-gray-100 font-bold duration-500 transition ease-in-out`}>Teklif
                        İste
                    </button>
                    {!visible ? null :
                        <div className='visible mt-1'>
                            <form action="https://getform.io/f/9e91b82f-b465-4863-88c1-253fd4c6ad1f" method="POST">
                                <input name={"fullName"}
                                       className={`bg-white border-2 border-[${props.data.colorCode}] focus:outline-none focus:border-[${props.data.colorCode}] w-full text-gray-700 mt-2 px-2 py-2`}
