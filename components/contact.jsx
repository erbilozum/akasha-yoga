        ...
        <div name={"contact"} className={"container mx-auto text-gray-400 select-none items-center"}>
            ...
            < div className={"font-NatoSans"}>
                    ...
                    <div className={"flex flex-row space-x-2 ml-4 text-2xl items-center justify-center"}>
                        <div className={"hover:scale-110 duration-500 transition-all ease-in-out"}>
                            <a href={data.facebookURL} target={"_blank"} rel={"noopener noreferrer"}>
                                <Image src={"/logos/logo-facebook.png"} alt={"facebook"} title={"facebook"} width={30}
                                       height={30}/>
                            </a>
                        </div>
                        <div className={"hover:scale-110 duration-500 transition-all ease-in-out"}>
                            <a href={data.instagramURL} target={"_blank"} rel={"noopener noreferrer"}>
                                <Image src={"/logos/logo-instagram.png"} alt={"instagram"} title={"instagram"}
                                       width={30} height={30}/>
                            </a>
                        </div>
                        <div className={"hover:scale-110 duration-500 transition-all ease-in-out"}>
                            <a href={data.youtubeURL} target={"_blank"} rel={"noopener noreferrer"}>
                                <Image src={"/logos/logo-youtube.png"} alt={"youtube"} title={"youtube"} width={30}
                                       height={30}/>
                            </a>
                        </div>
                    </div>
                ...
                <form action="https://getform.io/f/9e91b82f-b465-4863-88c1-253fd4c6ad1f" method="POST"
                    ...
                    <input type={"text"}
                           className={"w-full focus:outline-none sm:w-2/6 p-2 bg-white border-fuchsia-800 border-2 select-text"}
                           placeholder={"Full Name"} required/>
                    <input type={"email"}
                           className={"w-full focus:outline-none sm:w-2/6 p-2 bg-white border-fuchsia-800 border-2 select-text"}
                           placeholder={"info@somebody.com"} required/>
                    <input type={"tel"}
                           className={"w-full focus:outline-none  sm:w-2/6 p-2 bg-white  border-fuchsia-800 border-2 select-text"}
                           placeholder={"0 (123) 456 78 90"} required/>
                    <input type={"category"} value={"Contact Request"} className={"hidden"}
                           placeholder={"0 (123) 456 78 90"} required/>
                    <div className={"flex justify-end w-full sm:w-1/3"}>
                        <button type={"submit"}
                                className={"bg-fuchsia-800 hover:bg-fuchsia-700 duration-500 transition-all ease-in-out text-white tracking-wider px-5 py-2 rounded"}>Kayıt
                            Ol
                        </button>
                    </div>
                </form>
            ...
            < div className={"flex flex-col font-NotoSans"}>
                <div>
                    ...
                    <p className={"text-sm text-center"}>Eğer telefon ile ulaşamazsanız lütfen sms, WhatsApp veya
                        e-posta ile iletişime geçiniz.</p>
                </div>
                <div className={"flex flex-col sm:flex-row mt-10 mb-10 text-center justify-center"}>
                    <div
                        className={"flex flex-col font-bold text-xs w-full text-center sm:text-start sm:w-1/6 uppercase"}>
            ...
            {/*Google Maps*/}
            < div className={"mt-10"}>
                <iframe src={data.googleMapsURL} style={{border: 0, width: "100%", height: "40vh"}}
